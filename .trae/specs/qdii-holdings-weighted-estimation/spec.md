...
美股相关估算 SHALL 以"最新确认净值"为基准，结合最新美股交易日 T（解析自 us 指数时间戳/最新确认净值日期）计算增量。**T 的前一交易日为"前一个美股交易日"（跳过周末/美股节假日，非自然日减一）**：
- `D == T`：美股部分增量=0（白天恒定，等于展示确认净值）
- `D == T 的前一美股交易日`：增量=美股指数最近涨跌幅（确认净值未包含的最近交易日）
- `D < 前一交易日`：不叠加，估算回退"展示最新确认净值"

#### Scenario: 012753 确认净值滞后 1 个美股交易日（D==T前一交易日，跨周末）

- **WHEN** 012753 最新确认净值 08-28（周五，D），美股最新交易日 08-31（周一，T）
- **THEN** T 的前一美股交易日 = 08-28（跳过周末 08-29/30）== D → 白天估算增量 ≈ 美股 08-31 涨跌，而非误置 0
