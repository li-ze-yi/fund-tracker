...
- **ICP 备案**：部署在中国大陆服务器的域名必须完成 ICP 备案，否则手机浏览器无法访问（云服务商对未备案域名进行网络层拦截，电脑可能因缓存暂时可访问，但手机浏览器请求会被拦截导致 `ERR_CONNECTION_CLOSED`）。备案需 1-3 周，建议提前申请
...
> - [ ] 添加成功后，若累计收益为0（首次买入），自动生成一条买入交易记录；若累计收益不为0（存量持仓录入），不生成交易记录
...
>   - [ ] 当日收益（确认净值可用时）= 持有份额 × (今日确认净值 - 昨日确认净值)
>   - [ ] 当日收益（盘中估算时）= 持仓金额 × 估算涨幅 / (1 + 估算涨幅)
> - [ ] 累计收益 = 持仓金额 - 累计投入成本
...
> **Story 3.5**：作为用户，我想要修改已有持仓的金额和累计收益，以便在持仓信息有误时进行更正。
>
> **Acceptance Criteria**：
> - [ ] 正常路径：基金详情页 → 点击修改持仓 → 弹出修改持仓弹窗 → 修改持仓金额和/或累计收益 → 确认修改 → 修改成功
> - [ ] 弹窗自动回填当前市值和当前累计收益
> - [ ] 修改逻辑与添加持仓逻辑一致：shares = amount / netValue, totalCost = amount - totalReturn, costPrice = totalCost / shares
> - [ ] 修改持仓时不生成交易记录
> - [ ] 异常路径：持仓金额为空或 ≤ 0 → 提示"请输入有效的持仓金额"
...
> **Story 9.3**：作为用户，我想通过上传基金持仓截图自动识别并导入持仓，这样我不用手动逐个输入。
>
> **Acceptance Criteria**：
> - [ ] 支持上传 JPG/PNG 格式持仓截图（最大10MB）
> - [ ] 自动 OCR 识别基金名称/代码、持仓金额、累计收益
> - [ ] 识别结果可编辑修正（基金名称支持联动搜索）
> - [ ] 基金代码由系统匹配确定，不可手动修改
> - [ ] 支持选择导入分组
> - [ ] 主要支持支付宝持仓截图（测试阶段）
> - [ ] 识别不准确时用户可手动修正
...
```
1. 用户点击搜索框相机图标 → 打开图片导入弹窗
2. 上传持仓截图（拖拽/点击）→ 图片预览
3. 点击"开始识别" → OCR 识别（百度OCR优先，Tesseract.js兜底）
4. 显示识别结果列表（基金名称、代码、金额、累计收益）
5. 用户修正识别结果（基金名称可编辑+联动搜索）
6. 选择导入分组 → 确认导入
```
...
| note | VARCHAR(200) | DEFAULT NULL | 备注（自动定投标记：auto_plan:{planId}） |
| created_at | DATETIME | NOT NULL DEFAULT CURRENT_TIMESTAMP | 创建时间 |
...
| frequency | ENUM('daily','weekly','biweekly','monthly') | NOT NULL | 定投频率 |
...
2. 后端调用公开接口获取每个基金的实时估值数据（估算净值、估算涨幅）和历史净值数据
3. 计算衍生字段：
   - 持仓金额 = 持有份额 × 确认净值（effectiveNav）
   - 当日收益（确认净值可用时）= 持有份额 × (今日确认净值 - 昨日确认净值)  ← v2.10修正
   - 当日收益（盘中估算时）= 持仓金额 × 估算涨幅 / (1 + 估算涨幅)
   - 累计收益 = 持仓金额 - (持有份额 × 成本单价)
4. 返回聚合数据给前端
```
...
```
添加持仓（首次）：
1. 用户输入：持仓金额（当前市值）、累计收益、选择分组（可选）
2. 获取该基金最新净值（优先确认净值 → 实时估值回退）
3. 计算：
   - 持有份额 = 持仓金额 / 最新净值
   - 投入成本 = 持仓金额 - 累计收益
   - 成本单价 = 投入成本 / 持有份额
4. 写入 holdings 表（shares, cost_price, confirmed_nav, confirmed_nav_date, total_cost）
5. 若累计收益为0（!totalReturn），生成买入交易记录（type='buy', 份额=持有份额, 价格=最新净值, 金额=持仓金额, 日期=当天）
```
...
```
修改持仓：
1. 用户输入：持仓金额（当前市值）、累计收益
2. 获取该基金最新净值（优先确认净值 → 实时估值回退）
3. 计算：
   - 持有份额 = 持仓金额 / 最新净值
   - 投入成本 = 持仓金额 - 累计收益
   - 成本单价 = 投入成本 / 持有份额
4. 更新 holdings 表（shares, cost_price, total_cost）
5. 不生成交易记录
```
...
```
定投执行（定时任务，每日 20:00 + 21:00 重试）：
1. 查询所有 status='active' 且 next_run_date <= 今天 的定投计划
2. 对每个计划：
   a. 防重复检查：查询 transactions 中是否存在 note='auto_plan:{planId}' 且 transaction_date=今天的记录，已存在则跳过
   b. 获取该基金当日确认净值（历史净值接口查询今天的数据）
   c. 若净值未确认（查询结果为空），跳过本次执行，不更新 next_run_date，下次调度会再次拾取
   d. 计算买入份额 = 定投金额 / 确认净值（注意：plan.amount 等 Decimal 字段需 parseFloat() 转换）
   e. 生成交易记录（type='buy', note='auto_plan:{planId}'）
   f. 更新持仓（同加仓逻辑）
   g. 计算下次执行日期并更新 next_run_date
3. 返回执行统计 { success, skipped, failed, pending }
4. 定投不涉及真实支付，仅做持仓记录
5. 同一基金可有多个定投计划，单日可多次买入（通过 note 字段区分不同计划）
```
...
图片识别解析逻辑：
- OCR引擎：百度OCR（通用文字识别标准版）优先，Tesseract.js 兜底
- 支付宝截图解析：按中文行分组 → 识别名称续行（ETF联接C等）→ splitIndex分割续行前后金额
  - 持仓金额 = 名称续行前的最大正数金额
  - 累计收益 = 名称续行前的第二个金额
- 基金名称智能匹配：findBestMatch 函数，优先匹配C/A后缀 + 名称相似度评分
- 识别结果中基金代码为空时，通过 Fund.search(名称) 模糊查询数据库补全
```
...
| 深色模式 | ✅ 已支持 (v3.1) — 墨玉金深色主题 + 霜白碧浅色主题，默认浅色 |
...
CREATE TABLE IF NOT EXISTS transactions (
    ...
    note VARCHAR(200) DEFAULT NULL COMMENT '备注（自动定投标记）',
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
...
| 优先级 | 功能 | 描述 | 预计工作量 |
|--------|------|------|------------|
| P0 | ~~浅色主题~~ | ✅ 已实现 (v3.1) — 霜白碧浅色主题，默认启用，支持用户切换 | - |
...
#### E.2.2 状态设计方案（v2.11更新：5种状态）

| 时间段 | 数据来源 | 状态标记 | 颜色 | 动画 | 适用场景 |
|--------|---------|---------|------|------|---------|
| **🌙 17:00-22:30** | 当日**确认净值** | ✅ **已确认** | 🟡 浅金黄 #f5d584 | 静态 | 确认净值可用 |
| **☀️ 9:00-15:00** | 实时**估算值** | 📊 **估算中** | 🔴 红色 #EF4444 | 缓慢脉冲(3s) | 盘中实时估算 |
| **🌆 15:00-17:00** | 收盘**估算值** | ⏳ **待确认** | 🟠 橙色 #F97316 | 静态 | 收盘后等待确认 |
| **🌅 0:00-9:00（工作日）** | 昨日**确认净值** | 🔵 **待开市** | 🔵 蓝色 #60A5FA | 静态 | 盘前等待开盘，涨幅收益隐藏 |
| **⛔ 周末/节假日** | 最近**确认净值** | ⚫ **休市** | ⚫ 灰色 #6B7280 | 静态 | 非交易日 |

**颜色选择依据**（用户明确指定，v2.11更新）：
- ✅ 已确认 → **浅金黄** #f5d584 + 无脉冲动画（v2.4从绿色改为金黄）
- 📊 估算中 → **红色** #EF4444 + 缓慢脉冲动画（3秒周期）
- ⏳ 待确认 → **橙色** #F97316 + 静态
- 🔵 待开市 → **蓝色** #60A5FA + 静态（v2.11新增，盘前等待开盘）
- ⚫ 休市 → **灰色** #6B7280 + 静态（v2.4新增，周末/节假日）
...
**v2.4（4种状态）** ✅ 历史版本：
- 新增 `market_closed`（休市）灰色 #6B7280
- `confirmed` 颜色从绿色改为浅金黄 #f5d584

**v2.11（5种状态）** ✅ 当前版本：
- 新增 `pre_market`（待开市）蓝色 #60A5FA
- 盘前时段(hour < 9)显示"待开市"，涨幅收益隐藏
- 节假日检测优先于交易时间判断
...
|----------|----------|------|------|---------|
| **周末/节假日全天** | ⚫ 休市 | 灰色 #6B7280 | 静态 | 非交易日 |
| **00:00-09:00（工作日）** | 🔵 待开市 | 蓝色 #60A5FA | 静态 | 盘前等待开盘，涨幅收益隐藏 |
| **09:00-15:00** | 📊 估算中 | 🔴 红色 #EF4444 | ✨ 脉冲(3s) | 盘中实时估值 |
| **15:00-17:00** | ⏳ 待确认 | 🟠 橙色 #F97316 | 静态 | 收盘等待确认 |
| **17:00-22:30** | ✅ 已确认 | 🟡 浅金黄 #f5d584 | 静态 | 确认净值可用 |
...
2. yesterdayNav = historyData[1]?.nav || 0   ← v2.10新增：上一交易日净值
3. marketValue       = shares × effectiveNav
4. cumulativeReturn  = marketValue - totalCost
5. dailyProfit（确认净值可用时）= shares × (effectiveNav - yesterdayNav)  ← v2.10修正
   dailyProfit（盘中估算时）  = marketValue × gainPercent / (100 + gainPercent)
```
...
### 附录 H：隐私模式（隐藏金额）v2.11

#### H.1 功能概述

用户在公共场合查看持仓时，可一键隐藏所有金额和收益数据，避免隐私泄露。

#### H.2 用户故事

> 作为投资者，我希望在公共场合查看基金持仓时能隐藏金额数据，避免被旁人看到我的资产和收益信息。

#### H.3 交互方式

| 入口 | 操作 | 效果 |
|------|------|------|
| 持仓页 - 点击总资产金额 | 点击金额数字 | 切换显示/隐藏状态 |
| 设置页 - 隐私设置卡片 | Switch 开关 | 切换显示/隐藏状态 |
...
|------|----------|-----------|
| 持仓页 | 总资产、当日收益、累计收益 | `****` |
| 基金列表项（持仓模式） | 持仓金额、当日收益、累计收益 | `****` |
| 基金列表项（自选模式） | 净值、估算净值变动 | `****` |
| 收益统计页 | 总收益、最大盈利、最大亏损、收益金额列、累计收益列 | `****` |
| 定投计划页 | 定投金额 | `****` |

**不隐藏的数据**：估算涨幅（%）、收益率（%）、累计收益率（%）等百分比数据。

#### H.5 技术实现

- **状态管理**：`useHideAmountStore`（Zustand），`hidden: boolean`
- **持久化**：`localStorage('hide_amount')`，刷新页面不丢失
- **全局同步**：所有页面共享同一 store 实例，切换即时生效
- **条件渲染模式**：`hideAmount ? '****' : 原始金额`

#### H.6 不涉及的页面

基金详情页（FundDetailPage）不参与隐藏，交易记录金额和详情页数据始终明文显示。
...
| `web/src/components/modals/EditHoldingModal.tsx` | **新建** | 修改持仓弹窗，使用受控组件避免输入重置 |
| `web/src/services/holdingService.ts` | **签名变更** | updateHolding 参数从 totalCost/accumulatedProfit 改为 amount/totalReturn |
| `web/src/pages/fund/FundDetailPage.tsx` | **集成** | 集成 EditHoldingModal，传入当前市值和累计收益 |

---

## 附录 H：定投调度修复与防重复执行机制（v3.0, 2026-05-25 更新）

### H.1 背景与问题

v3.0 之前，定投计划存在以下问题：

1. **定投计划不生效**：缺少 cron 调度器，定投计划从未被自动执行
2. **calcNextRunDate 忽略用户指定日期**：weekly/monthly 频率未正确处理 dayOfWeek/dayOfMonth
3. **MySQL Decimal 类型导致 NaN**：plan.amount 等字段从 MySQL 返回 Decimal 字符串，JS + 运算触发字符串拼接
4. **15:30 净值未确认**：A股基金净值通常18:00-20:00确认，15:30执行时净值尚未更新
5. **重复执行**：多次测试或调度重复触发导致同一计划同一天多条交易记录

### H.2 解决方案概览

| 问题 | 解决方案 |
|------|---------|
| 定投不生效 | 添加 node-cron 调度器（20:00+21:00 双时间） |
| calcNextRunDate 错误 | 修复 dayOfWeek/dayOfMonth 参数处理 |
| Decimal NaN | parseFloat() 转换所有 Decimal 字段 |
| 净值未确认 | 只使用当日确认净值，未确认则跳过 |
| 重复执行 | note 字段防重复 + auto_plan:{planId} 标记 |

### H.3 数据库Schema变更

```sql
ALTER TABLE transactions ADD COLUMN note VARCHAR(200) DEFAULT NULL COMMENT '备注（自动定投标记）';
ALTER TABLE investment_plans MODIFY COLUMN frequency ENUM('daily','weekly','biweekly','monthly') NOT NULL;
```

### H.4 定投执行流程

```
20:00 cron 触发 executeDuePlans()
  ↓
查询 status='active' AND next_run_date <= CURDATE() 的计划
  ↓
对每个计划：
  ├── 防重复检查：SELECT id FROM transactions WHERE note='auto_plan:{planId}' AND transaction_date=CURDATE()
  │   └── 已存在 → skipped++
  ├── 获取当日确认净值：查询历史净值接口 today 的数据
  │   └── 未确认 → pending++（不更新 next_run_date）
  ├── parseFloat() 转换所有 Decimal 字段
  ├── 计算买入份额 = amount / netValue
  ├── 创建交易记录（note='auto_plan:{planId}'）
  ├── 更新持仓（同加仓逻辑）
  └── 计算并更新 next_run_date → success++
  ↓
返回 { success, skipped, failed, pending }
  ↓
21:00 cron 再次触发（处理 20:00 因净值未确认而跳过的计划）
```

### H.5 防重复执行机制

**标记方式**：transactions.note = `auto_plan:{planId}`

**查询逻辑**：
```sql
SELECT id FROM transactions
WHERE user_id = ? AND fund_code = ? AND type = 'buy'
AND transaction_date = ? AND note = ?
LIMIT 1
```

**特性**：
- 同一基金可有多个定投计划（不同 planId 产生不同 note）
- 单日可多次买入（不同计划的 note 不同）
- 同一计划同一天只执行一次
...
| 决策 | 选项 | 选择 | 理由 |
|------|------|------|------|
| 净值未确认策略 | A.降级用旧净值 B.跳过等待 | B | 保证准确性，避免用旧净值计算错误 |
| 防重复方式 | A.独立表 B.note字段 | B | 简单直接，复用现有字段 |
| 调度时间 | A.15:30 B.20:00+21:00 | B | A股净值18:00-20:00确认 |
| Decimal处理 | A.String() B.parseFloat() | B | 正确转换为数字类型 |

### H.7 文件变更清单

| 文件 | 变更类型 | 主要改动 |
|------|---------|---------|
| server/services/planService.js | 核心修改 | pool引入+日志+净值策略+parseFloat+防重复 |
| server/app.js | 修改 | cron调度器20:00+21:00+启动检查 |
| server/models/transaction.js | 修改 | create方法新增note参数 |
| server/controllers/planController.js | 修改 | calcNextRunDate修复+biweekly |
| doc/init_db.sql | 修改 | v3→v4+note字段+biweekly枚举 |

---

## 附录 I：图片识别导入基金持仓功能（v2.9）

### I.1 功能概述
通过上传基金持仓截图，自动 OCR 识别并导入持仓。

### I.2 技术架构
- 后端：百度OCR API + Tesseract.js 双引擎
- 前端：ImageImportModal 组件（三步流程：上传→识别→确认）
- 图片预处理：sharp（放大、灰度化、对比度增强、锐化）

### I.3 API 接口
| 接口 | 方法 | 说明 |
|------|------|------|
| /api/image-import/recognize | POST | 上传图片识别持仓 |
| /api/image-import/confirm | POST | 确认导入持仓 |
