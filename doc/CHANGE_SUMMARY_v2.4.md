...
|------|-------|-------|----------|----------|
| **盘中交易 (9:00-15:00)** | 20秒 | **28秒** | 1.4x | 估值波动相对缓慢，配合前端30秒刷新频率 |
...
|----------|-------|-------|----------|----------|
| **近期历史净值** (3天内) | 1小时 | **盘中30分钟 / 收盘后5分钟** | 动态 | 盘中确认延迟可接受；收盘后5分钟快速刷新 |
...
getTTL(type) {
  switch (type) {
    ...
    case 'history_recent':
      return 30 * 60 * 1000;     // 近期：盘中30分钟（收盘后5分钟，动态TTL）
...
|------|-----------------|-----------------|---------|
| **工作日盘中** | 18,000次 | 实际由forceRefresh决定（前端30秒刷新时穿透缓存） | **取决于forceRefresh** |
...
getRealtimeTTL() {
  switch (status) {
    case 'trading': return 28 * 1000;  // 盘中28秒（配合前端30秒刷新）
