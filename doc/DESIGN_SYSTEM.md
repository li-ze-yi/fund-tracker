...
- **浅色主色调**: 暖白 (#F7F8FA) + 鎏金 (#B8860B / #D4A84B) — 鎏金白
- **默认主题**: 鎏金白（浅色）
- **情感**: 可信赖、高效、现代
...
#### 鎏金 — 浅色主题 (`data-theme='light'`) ⭐ v3.3更新（由霜白碧改为暖金体系）

```css
:root[data-theme='light'] {
  /* 品牌色系 */
  --accent-gold: #B8860B;              /* 品牌主色 - 鎏金（暖金） */
  --accent-gold-light: #D4A84B;        /* 品牌亮色 - 亮金 */
  --accent-gold-dim: rgba(184, 134, 11, 0.12);  /* 品牌弱色 - 用于背景填充 */

  /* 功能色 */
  --gain: #DC2626;                     /* 收益/上涨 - 红色 */
  --loss: #16A34A;                     /* 亏损/下跌 - 绿色 */
  --gain-bg: rgba(220, 38, 38, 0.06);   /* 收益背景 */
  --loss-bg: rgba(22, 163, 74, 0.06);    /* 亏损背景 */
  --gain-border: rgba(220, 38, 38, 0.18);  /* 收益边框 */
  --loss-border: rgba(22, 163, 74, 0.18);  /* 亏损边框 */
}
```
...
项目采用双主题设计：**墨玉金**（深色）和**鎏金白**（浅色），默认使用鎏金白（浅色）主题。

- **墨玉金** (`data-theme='dark'`)：深蓝黑底 + 金色品牌色，适合暗光环境
- **鎏金白** (`data-theme='light'`)：暖白底 + 鎏金品牌色，适合明亮环境，**默认主题**
...
| v3.3 | 2026-09-04 | 品牌色统一为金色体系：浅色主题由"霜白碧（青玉）"更正为"鎏金（暖金 #8F6A06/#A5710A）"，与实现一致；新增全局氛围背景（金色光晕+噪点）、暖色阴影、金色辉光与主按钮渐变令牌；金色色值按 WCAG AA 对比度校准（主色浅底 4.62:1、按钮白字 ≥3:1） |
