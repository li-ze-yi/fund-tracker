...
function parseAlipayByNamePattern(text) {
  ...
  for (const group of mergedGroups) {
    ...
    // 真实支付宝布局（OCR 逐行读出，以实际截图验证为准）：
    //   名称前半 | 金额 | 持有收益(累计收益) | 名称续行 | 昨日收益 | 收益率
    // - 名称续行前（preAmounts）：金额、持有收益(累计收益，可能为负)
    // - 名称续行后（postAmounts）：昨日收益
    // 因此累计收益取 preValues[1]，续行后的金额是"昨日收益"，不属于累计收益。
    let totalReturn = 0;
